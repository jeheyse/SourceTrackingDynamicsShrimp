  ...
  for(null.one in 1:(nrow(spXsite)-1)){
    for(null.two in (null.one+1):nrow(spXsite)){
      ...
      for(i in 1:reps){
        ...
        ##calculate null bray curtis (ADJUSTED THIS LINE FROM ORIGINAL CODE OF Stegen et al. 2013)
        # null_bray_curtis[i] = distance(null.spXsite,method='bray-curtis'); # ORIGINAL
        null_bray_curtis[i] = vegdist(null.spXsite, method='bray')
        
      }; # end reps loop
      
      ## empirically observed bray curtis (ADJUSTED THIS LINE FROM ORIGINAL CODE OF Stegen et al. 2013)
      # obs.bray = distance(spXsite[c(null.one,null.two),],method='bray-curtis'); # ORIGINAL
